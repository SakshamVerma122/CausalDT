...
# Designed for handling and processing a dataset within the context of a decision tree, particularly for causal analysis.
# To identify and retain features that are significantly correlated with the outcome variable.
class MyRows:
...
def _main():
    ...
    # parser.add_argument call adds a specific argument that the user can pass to the script from the command line.
    # if the decision tree is more ellaborated then we should reduce the the spilts and decrease the height 
    # I believe that the we can decide it on the basis of histograms
    # You can also see WCSS
    parser.add_argument(
        "--sp",
        default=3,
        type=int,
    ...
    if random_flag:
        ...
        # Getting random rows from the dataframe
        target = hsi_df.loc[random.choices(
            list(hsi_df.index), k=random_pick), :]
    else:
        target = hsi_df
    
    # printing length of the dataframe and the dependent column name
    ...
    # number of rows in that dataframe
