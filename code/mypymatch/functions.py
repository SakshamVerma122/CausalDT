...
def drop_static_cols(df, yvar, cols=None):
    ...
    # df: DataFrame from which to remove static columns.
    # yvar: The column to be excluded from the removal process.
    # cols (optional): Specific columns to consider for removal.
    
    # It will go inside the for loop when cols is None and no list of columns are provided to drop_static_cols
    if not cols:
    ...
    for col in df[cols]:
        ...
        # if the col in consideration have only one value then it should be removed
        if n_unique == 1:
            df.drop(col, axis=1, inplace=True)

    return df

# Compares two distributions using the KS test over multiple bootstrap samples 
def ks_boot(tr, co, nboots=1000):

    # tr, co: Two arrays for which the KS test is to be performed.
